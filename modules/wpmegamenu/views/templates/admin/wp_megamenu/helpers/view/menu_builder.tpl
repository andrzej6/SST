{if $wpmmMenuItems.items}

    {$depth = 1}
    <ol class="sortable">

        {foreach $wpmmMenuItems.items as $menuItem}
            ...
            {if ($menuItem@index != $wpmmMenuItems.count - 1) && ($wpmmMenuItems.items[($menuItem@index + 1)]->depth <= $menuItem->depth)}
                </li>
            {/if}

            {$depth = $menuItem->depth}

            {if ($menuItem@index == $wpmmMenuItems.count - 1)}
