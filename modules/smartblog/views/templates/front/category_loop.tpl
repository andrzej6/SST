<div itemtype="#" itemscope="" class="sdsarticleCat clearfix">
    ...
    <div class="sdsarticleHeader">
         ...
         <span>{l s='Posted by' mod='smartblog'} 
             <span itemprop="author">{if $smartshowauthor ==1}&nbsp;<i class="icon icon-user"></i>&nbsp; {if $smartshowauthorstyle != 0}{$post.firstname} {$post.lastname}{else}{$post.lastname} {$post.firstname}{/if}
             </span>{/if} &nbsp;&nbsp;<i class="icon icon-tags"></i>&nbsp; 
             
             <span itemprop="articleSection"><a href="{smartblog::GetSmartBlogLink('smartblog_category',$catlink)}" class="blog-catlinks">{if $title_category != ''}{$title_category}{else}{$post.cat_name}{/if}</a>
             </span> &nbsp;
             
             <span class="comment"> &nbsp;<i class="icon icon-comments"></i>&nbsp; <a title="{$post.totalcomment} Comments" href="{smartblog::GetSmartBlogLink('smartblog_post',$options)}#articleComments" class="blog-catlinks">{$post.totalcomment} {l s=' Comments' mod='smartblog'}</a>
             </span>{if $smartshowviewed ==1}&nbsp; <i class="icon icon-eye-open"></i>{l s=' views' mod='smartblog'} ({$post.viewed}){/if}
    ...
    <div class="articleContent">
          <a itemprop="url" title="{$post.meta_title}" class="imageFeaturedLink">
                    ...
                    {if ($post.post_img != "no" && $activeimgincat == 0) || $activeimgincat == 1}
                        {if ($post.meta_title=='Our Trip to Clerkenwell Design Week (CDW)')}
                            <img itemprop="image" alt="{$post.meta_title}" src="/img/blog/desk-history.jpg" class="imageFeatured">
