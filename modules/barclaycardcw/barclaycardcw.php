...
require_once $modulePath . '/barclaycardcw/lib/loader.php';
...
require_once 'BarclaycardCw/Entity/ExternalCheckoutContext.php';
require_once 'BarclaycardCw/SmartyProxy.php';
require_once 'BarclaycardCw/Entity/Transaction.php';



require_once $modulePath . '/barclaycardcw/lib/BarclaycardCw/TranslationResolver.php';
 ...
 * BarclaycardCw
 *
 * This class defines all central vars for the BarclaycardCw modules.
 *          	 		 	 	 	   
 * 
 ...
 * class BarclaycardCw extends BarclaycardCw_PaymentMethod {
 *
...
class BarclaycardCw extends Module {
	...
	public function __construct(){
		...
		$this->tab = 'checkout';
		$this->version = preg_replace('([^0-9\.a-zA-Z]+)', '', '2.1.236');
		...
		if (Module::isInstalled($this->name)) {
			$migration = new Customweb_Database_Migration_Manager(BarclaycardCw_Util::getDriver(), dirname(__FILE__) . '/updates/', 
					_DB_PREFIX_ . 'barclaycardcw_schema_version');
			$migration->migrate();
		}
		
		// The parent construct is required for translations          	 		 	 	 	   
		$this->displayName = $this->l('DISPLAY NAME');
		$this->description = $this->l('ACCEPTS PAYMENTS MAIN');
		...
		if (!isset($_GET['configure']) && $this->context->controller instanceof AdminModulesController && method_exists('Module', 'isModuleTrusted') &&
			...
			$GLOBALS['cwrmUnTrustedMs'][] = 'barclaycardcw';
		}
	...
	private function checkLicense(){
		...
		$arguments = null;
		return Customweb_Licensing_BarclaycardCw_License::run('v8emklal82gikunn', $this, $arguments);
	}

	public function call_770nbimk4ukrhu4p() {
	...
	public static function getInstance(){
		if (self::$instance === null) {
			self::$instance = new BarclaycardCw();
		}
	...
	public function install(){
		$this->installController('AdminBarclaycardCwRefund', 'Barclaycard Refund');
		$this->installController('AdminBarclaycardCwMoto', 'Barclaycard Moto');
	...
	public function uninstall(){
		$this->uninstallController('AdminBarclaycardCwRefund');
		$this->uninstallController('AdminBarclaycardCwMoto');
	...
	private function installConfigurationValues(){
		...
		$this->getConfigApi()->updateConfigurationValue('MOBILE_TEMPLATE_METHOD', 'default');
		$this->getConfigApi()->updateConfigurationValue('MOBILE_TEMPLATE_URL', '');
	...
	private function uninstallConfigurationValues(){
		...
		$this->getConfigApi()->removeConfigurationValue('MOBILE_TEMPLATE_METHOD');
		$this->getConfigApi()->removeConfigurationValue('MOBILE_TEMPLATE_URL');
	...
	private function getConfigurationForm(){
		...
		$forms = array(
			array(
				'form' => array(
					'legend' => array(
						'title' => 'Barclaycard: ' . $this->l('Settings'),
						'icon' => 'icon-envelope' 
					),
					'input' => $fields,
					'submit' => array(
	...
	protected function getFormFields(){
		$fields = array(
			...
			"),
 			...
 			13 => array(
				'name' => 'MOBILE_TEMPLATE_METHOD',
 				'label' => $this->l("Mobile Template"),
 				'desc' => $this->l("With this option you can control which template is used
				for mobile devices. Per default the template for desktops is used.
				You can either use a own template (provided through the field below)
				or the customweb hosted template.
			"),
 				'default' => 'default',
 				'type' => 'select',
 				'options' => array(
					'query' => array(
						0 => array(
							'id' => 'default',
 							'name' => $this->l("Use standard template (as selected above)
				"),
 						),
 						1 => array(
							'id' => 'custom',
 						...
 						2 => array(
							'id' => 'customweb',
 							'name' => $this->l("Use customweb hosted template"),
 						),
 					),
 					'name' => 'name',
 					'id' => 'id',
 			...
 			14 => array(
				'name' => 'MOBILE_TEMPLATE_URL',
 				'label' => $this->l("Mobile Template URL"),
 				'desc' => $this->l("The URL indicated here is used to rendered the template
				for
				mobile devices. This field is only considered when the option
				mobile template is set to 'Use own template'.
			"),
 				'default' => '',
 				'type' => 'text',
 			),
	...
	public function hookDisplayBeforeShoppingCartBlock(){
		...
		$contextId = Tools::getValue('barclaycardcw-context-id', null);
		if ($contextId !== null) {
	...
	private function handleChangesForAuthController(){
		...
		if ($this->context->controller instanceof AuthController) {
			$contextId = Tools::getValue('barclaycardcw-context-id');
		}
		if (empty($contextId) && isset($cookie->barclaycardcw_context_id)) {
			$contextId = $cookie->barclaycardcw_context_id;
		}
		...
		if (!empty($contextId)) {
			...
			if ($cookie->logged == 1 && !empty($cookie->email)) {
				unset($cookie->barclaycardcw_context_id);
				$context->setCustomerEmailAddress($cookie->email);
				BarclaycardCw_Util::getEntityManager()->persist($context);
				header('Location: ' . $context->getAuthenticationSuccessUrl());
				exit();
			}
			$cookie->barclaycardcw_context_id = $contextId;
			$link = new Link();
			$url = $link->getPageLink('authentication', true, null, 
					array(
						'barclaycardcw-context-id' => $context->getContextId(),
			...
			if ($context->getBillingAddress() !== null) {
				...
				if (!isset($_POST['id_gender'])) {
					$_POST['id_gender'] = BarclaycardCw_Util::getGenderId($context->getBillingAddress()->getGender());
				}
	...
	private function loadContext($contextId){
		if (!empty($contextId)) {
			return BarclaycardCw_Util::getEntityManager()->fetch('BarclaycardCw_Entity_ExternalCheckoutContext', $contextId);
		}
		else {
	...
	private function checkToken(BarclaycardCw_Entity_ExternalCheckoutContext $context){
		if (!empty($_GET['barclaycardcw-context-id'])) {
	...
	public function hookCwExternalCheckoutCollection(){
		...
		$context = BarclaycardCw_Entity_ExternalCheckoutContext::getReusableContextByCartId($this->context->cart->id);
		if ($context === null) {
			$context = new BarclaycardCw_Entity_ExternalCheckoutContext();
			BarclaycardCw_Util::getEntityManager()->persist($context);
		}
		$providerService = BarclaycardCw_Util::createContainer()->getBean('Customweb_Payment_ExternalCheckout_IProviderService');
		...
		foreach ($checkouts as $checkout) {
			...
			$widgets['barclaycardcw_' . $checkout->getMachineName()] = array(
	...
	public function hookBackOfficeHeader(){
		...
		if ((isset($_POST['partialRefund']) || isset($_POST['cancelProduct'])) && !isset($_GET['confirmed'])) {
			$transaction = current(BarclaycardCw_Entity_Transaction::getTransactionsByOrderId($id_order));
			if (is_object($transaction) && $transaction->getTransactionObject() !== null &&
				...
				if ($order->module == ('barclaycardcw_' . $transaction->getPaymentMachineName())) {
					$url = '?controller=AdminBarclaycardCwRefund&token=' . Tools::getAdminTokenLite('AdminBarclaycardCwRefund');
					$url .= '&' . Customweb_Http_Url::parseArrayToString($_POST);
					header('Location: ' . $url);
					die();
				}
			}
		...
		if (isset($_POST['submitBarclaycardCwRefundAuto'])) {
			try {
				$transaction = current(BarclaycardCw_Entity_Transaction::getTransactionsByOrderId($id_order));
				$this->refundTransaction($transaction->getTransactionId(), self::getRefundAmount($_POST));
			}
			catch (Exception $e) {
				$this->context->controller->errors[] = $this->l("Could not refund the transaction: ") . $e->getMessage();
		...
		if (isset($_GET['controller']) && $_GET['controller'] == 'AdminOrders' && isset($_POST['submitAddOrder']) && !isset($_GET['confirmed'])) {
			...
			if (substr($paymentMethodName, 0, strlen('barclaycardcw')) == 'barclaycardcw') {
				$url = '?controller=AdminBarclaycardCwMoto&token=' . Tools::getAdminTokenLite('AdminBarclaycardCwMoto');
				$url .= '&' . Customweb_Http_Url::parseArrayToString($_POST);
				header('Location: ' . $url);
				die();
			}
	...
	public function hookAdminOrder($params){
		...
		if (!strstr($order->module, 'barclaycardcw')) {
			return '';
		}
		...
		$transactions = BarclaycardCw_Entity_Transaction::getTransactionsByCartOrOrder($order->id_cart, $order->id);
	...
	private function processAdminAction(){
		if (isset($_POST['id_transaction'])) {
			...
			if (isset($_POST['submitBarclaycardCwRefund'])) {
				$amount = null;
			...
			if (isset($_POST['submitBarclaycardCwCancel'])) {
			...
			if (isset($_POST['submitBarclaycardCwCapture'])) {
				$amount = null;
	...
	public function refundTransaction($transactionId, $amount = null, $close = false){
		...
		if ($dbTransaction->getTransactionObject() != null && $dbTransaction->getTransactionObject()->isRefundPossible()) {
			...
			BarclaycardCw_Util::getEntityManager()->persist($dbTransaction);
		}
		else {
	...
	public static function translate($string, $sprintf = false, $module = 'barclaycardcw'){
...
if (!function_exists('cwSmartyTranslate')) {
	...
	function cwSmartyTranslate($params, &$smarty){
		...
		return BarclaycardCw::translate($params['s'], $sprintf, $params['mod']);
	}
