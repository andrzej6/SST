 ...
 *         	 		 	 	 	   
 */
class AdminBarclaycardCwMotoController extends AdminController
{
	public function __construct() {
		$this->className = 'AdminBarclaycardCwMotoController';
		parent::__construct();
		$this->context->smarty->addTemplateDir($this->getTemplatePath());
		$this->tpl_folder = 'barclaycardcw_moto/';
		$this->bootstrap = true;
	}
	...
	public function initContent()
	{
		...
		if (isset($_GET['cw_transaction_id'])) {
			...
			$dbTransaction = BarclaycardCw_Entity_Transaction::loadById($_GET['cw_transaction_id']);
			if ($dbTransaction->getTransactionObject()->isAuthorized()) {
				...
				$params['vieworder'] = '1';
				header('Location: ' . BarclaycardCw::getAdminUrl('AdminOrders', $params));
				die();
			}
			else {
		...
		if ($failedTransaction !== NULL) {
			$moduleInstance = BarclaycardCw_PaymentMethod::getInstanceById($dbTransaction->getModuleId());
		}
		else {
		...
		/* @var $moduleInstance BarclaycardCw_PaymentMethod */
		...
		$adapter = BarclaycardCw_Util::getAuthorizationAdapter(Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME);
		...
		if ($isSupported) {
			...
			if ($visibleFormFields !== null && count($visibleFormFields) > 0) {
				$renderer = new BarclaycardCw_FormRenderer($moduleInstance->getPaymentMethodName());
