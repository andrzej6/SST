...
class BarclaycardCwSuccessModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function initContent()
	{
		...
		$dbTransaction = BarclaycardCw_Entity_Transaction::loadById(intval($transactionId));
		...
		$timeoutUrl = $link->getModuleLink('barclaycardcw', 'timeout', array('cw_transaction_id' => $dbTransaction->getTransactionId()), true);
		
		$failedUrl = $link->getModuleLink('barclaycardcw', 'error', array(
		...
		$cookieKey = 'alias_barclaycardcw_' . $dbTransaction->getModuleId();
		...
		while (true) {
			...
			$dbTransaction = BarclaycardCw_Entity_Transaction::loadById(intval($transactionId), false);
