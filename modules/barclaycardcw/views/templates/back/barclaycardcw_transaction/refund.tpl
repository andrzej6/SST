		...
		<div class="pageTitle">
			<h3>
				<span id="current_obj" style="font-weight: normal;"> 
					...
					<span class="breadcrumb item-2 ">
						{lcw s='Refund' mod='barclaycardcw'}
					</span>
				</span>
			</h3>
		</div>
...
{if $transaction->getTransactionObject()->isPartialRefundPossible()}
	<form method="POST" class="barclaycardcw-line-item-grid" id="refund-form">
		...
		<table class="table table-striped table-condensed table-hover table-bordered">
			...
			<tfoot>
				<tr>
					<td colspan="6" class="text-right">{lcw  mod='barclaycardcw' s='Total Refund Amount'}:</td>
		...
		{if $transaction->getTransactionObject()->isRefundClosable()}
			<div class="closable-box">
				<label for="close-transaction">{lcw  mod='barclaycardcw' s='Close transaction for further refunds'}</label>
				<input id="close-transaction" type="checkbox" name="close" value="on" />
			</div>
		{/if}
		
		<div class="text-right">
			<input type="submit" class="button btn btn-success" value="{lcw  mod='barclaycardcw' s='Refund'}" />
		</div>
	</form>
{/if}
