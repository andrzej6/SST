...
class BarclaycardCw_OrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext
{
	private $cartId;
	private $customerId;
	private $carrierName;
	...
	public function __construct($cart, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		...
		$this->languageCode = $lang['language_code'];
		...
		if (method_exists($cart, 'getFee') && $cart->feeamount <= 0) {
		...
		$key = BarclaycardCw_Util::getCheckoutCookieKey($paymentMethod);
		$cookie = Context::getContext()->cookie;
		if (!isset($cookie->{$key}) || $cookie->{$key} === null) {
			$cookie->{$key} = Customweb_Core_Util_Rand::getUuid();
		}
		$this->checkoutId = $cookie->{$key};
		$this->employeeId = BarclaycardCw_Util::getEmployeeIdFromCookie();
	}
	...
	private function buildLineItems(Cart $cart, $orderTotal) {
		return BarclaycardCw_Util::createLineItemsFromCart($cart, $orderTotal, $this->getPaymentMethod());
	}
