...
require_once 'BarclaycardCw/Entity/PaymentCustomerContext.php';
require_once 'BarclaycardCw/DatabaseLinkAccessor.php';
require_once 'BarclaycardCw/EntityManager.php';



final class BarclaycardCw_Util {
	...
	public static function getCheckoutCookieKey(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		return 'barclaycardcw_checkout_id' . $paymentMethod->getPaymentMethodName();
	}
	...
	public static function getPaymentCustomerContext($customerId) {
		...
		if (!isset(self::$paymentCustomerContexts[$customerId])) {
			$entities = self::getEntityManager()->searchByFilterName('BarclaycardCw_Entity_PaymentCustomerContext', 'loadByCustomerId', array(
	...
	public static function createContainer() {
		if (self::$container === null) {
			...
			$packages[] = 'BarclaycardCw_LayoutRenderer';
			$packages[] = 'BarclaycardCw_ExternalCheckoutService';
			$packages[] = 'BarclaycardCw_EndpointAdapter';
			...
			$storageBackend = new Customweb_Storage_Backend_Database(self::getEntityManager(), self::getDriver(), 'BarclaycardCw_Entity_Storage');
			$provider
			...
			->add('databaseTransactionClassName', 'BarclaycardCw_Entity_Transaction');
	...
	public static function getEndpointDispatcher() {
		return new Customweb_Payment_Endpoint_Dispatcher(self::createContainer()->getBean('BarclaycardCw_EndpointAdapter'), self::createContainer(), array(
			0 => 'Customweb_Barclaycard',
 			1 => 'Customweb_Payment_Authorization',
 		));
	}
	...
	public static function getDriver() {
		if (self::$driver === null) {
			...
			$link = BarclaycardCw_DatabaseLinkAccessor::getBarclaycardCwLink($databaseInstance);
	...
	public static function getAssetResolver() {
		...
		if (!isset(self::$resolvers[$shopId])) {
			...
			$moduleUrl = $context->shop->getBaseURL() . 'modules/barclaycardcw/';
				
			$currentTemplatePath = _PS_ALL_THEMES_DIR_ . $context->shop->getTheme() . '/modules/barclaycardcw/';
			$currentTemplateUrl = $context->shop->getBaseURL() . '/themes/' . $context->shop->getTheme() . '/modules/barclaycardcw/';
	...
	public static function extractMailVariables(Customweb_Payment_Authorization_ITransaction $transaction) {
		...
		$variables['{barclaycardcw_transaction_id}'] = $transaction->getTransactionId();
		$variables['{barclaycardcw_payment_id}'] = $transaction->getPaymentId();
		foreach ($transaction->getTransactionLabels() as $labelKey => $label) {
			$variables['{barclaycardcw_label_' . $labelKey . '}'] = $label['value'];
		}
		
		if ($transaction->getPaymentInformation() !== null) {
			$variables['{barclaycardcwpayment_information}'] = $transaction->getPaymentInformation();
		}
	...
	public static function getOrderCreationMessage($id_employee) {
		...
		if (isset($id_employee) && $id_employee > 0) {
			...
			$message = BarclaycardCw::translate('Manual order -- Employee:');
	...
	public static function createLineItemsFromCart(Cart $cart, $orderTotal, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod = null) {
		...
		if ($wrappingCosts > 0) {
			$taxRate = ($shippingCosts - $wrappingCostExcl) / $wrappingCostExcl * 100;
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem(
					'wrapping',
						BarclaycardCw::translate("Wrapping Fee"),
					$taxRate,
					$wrappingCosts,
					1,
		...
		if (method_exists($cart, 'getFee') && $paymentMethod != null) {
			$cart->getFee('barclaycardcw_' . $paymentMethod->getPaymentMethodName());
			$feeamount = $cart->feeamount;
			if ($feeamount != 0) {
				$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem(
						'payment-fee',
						BarclaycardCw::translate("Payment Fee"),
						0,
						$feeamount,
						1,
