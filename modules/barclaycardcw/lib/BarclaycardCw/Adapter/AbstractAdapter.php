...
require_once 'BarclaycardCw/Adapter/IAdapter.php';
require_once 'BarclaycardCw/Util.php';
require_once 'BarclaycardCw/Entity/Transaction.php';
require_once 'BarclaycardCw/FormRenderer.php';
...
abstract class BarclaycardCw_Adapter_AbstractAdapter implements BarclaycardCw_Adapter_IAdapter {
	...
	/**
	 * @var BarclaycardCw_Entity_Transaction
	 */
	...
	public function isHeaderRedirectionSupported() {
		...
		$arguments = null;
		return Customweb_Licensing_BarclaycardCw_License::run('mlghmnlv6lh67uc9', $this, $arguments);
	}

	public function call_jvp4itbsau4j6fue() {
	...
	public function handleAliasTransaction(BarclaycardCw_PaymentMethod $paymentMethod, Customweb_Payment_Authorization_IOrderContext $orderContext) {
		...
		if (BarclaycardCw_Util::isAliasManagerActive($orderContext)) {
			$moduleId = Tools::getValue('id_module', NULL);
			$cookieKey = 'alias_barclaycardcw_' . $this->paymentMethod->id;
			if ($moduleId == $this->paymentMethod->id) {
				$createNewAlias = Tools::getValue('barclaycardcw_create_new_alias', 'off');
				$createNewAliasCheckBoxPresent = Tools::getValue('barclaycardcw_create_new_alias_present', 'inactive');
				$alias = Tools::getValue('barclaycardcw_alias', NULL);
				$useNewCard = Tools::getValue('barclaycardcw_alias_use_new_card', NULL);
				$useStoredCard = Tools::getValue('barclaycardcw_alias_use_stored_card', NULL);
			...
			if (isset($this->context->cookie->{$cookieKey}) && !empty($this->context->cookie->{$cookieKey})) {
				...
				$aliasTransactions = BarclaycardCw_Util::getAliasHandler()->getAliasTransactions($orderContext);
			...
			if ($this->aliasTransactionId !== null && $this->aliasTransactionId !== 'new') {
				$this->aliasTransaction = BarclaycardCw_Entity_Transaction::loadById((int)$this->aliasTransactionId);
			}
		}
		
	}
	
	public function prepareCheckout(BarclaycardCw_PaymentMethod $paymentMethod, Customweb_Payment_Authorization_IOrderContext $orderContext, $failedTransaction, $createTransaction) {
		
		if ($failedTransaction !== null & !($failedTransaction instanceof BarclaycardCw_Entity_Transaction)) {
			throw new Exception("The failed transaction is not of instance BarclaycardCw_Entity_Transaction.");
		}
	...
	protected function getPaymentCustomerContext() {
		return BarclaycardCw_Util::getPaymentCustomerContext($this->getOrderContext()->getCustomerId());
	}
	
	protected function executeValidation() {
		$this->getInterfaceAdapter()->validate($this->getOrderContext(), BarclaycardCw_Util::getPaymentCustomerContext($this->getOrderContext()->getCustomerId()), $this->getFormData());
	}
	...
	public function getCheckoutPageHtml() {
		return $this->getPaymentFormPane();
	}
	...
	protected function getAliasForm() {
		...
		$aliasTransactions = BarclaycardCw_Util::getAliasHandler()->getAliasTransactions($this->getOrderContext());
	...
	protected function getPaymentFormPaneVariables() {
		...
		if ($visibleFormFields !== null && count($visibleFormFields) > 0) {
			$renderer = new BarclaycardCw_FormRenderer($this->paymentMethod->getPaymentMethodName());
	...
	protected function getPaymentFormPane() {
		$this->smarty->assign($this->getPaymentFormPaneVariables());
		return $this->renderTemplate('form/pane.tpl');
	}
	
	protected function persistTransaction() {
		if ($this->getTransaction() !== null) {
			BarclaycardCw_Util::getEntityManager()->persist($this->getTransaction());
		}
	}
	...
	protected function getBaseVariables() {
		require_once 'Customweb/Licensing/BarclaycardCw/License.php';
		$arguments = null;
		return Customweb_Licensing_BarclaycardCw_License::run('aluh9c30dpbk0g2s', $this, $arguments);
	}

	public function call_20rrnd8ijssf80kq() {
		$arguments = func_get_args();
		$method = $arguments[0];
		$call = $arguments[1];
		$parameters = array_slice($arguments, 2);
	...
	public function l($string, $specific = false) {
		return BarclaycardCw::translate($string);
	}
}