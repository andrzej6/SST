...
require_once 'BarclaycardCw/Adapter/AbstractAdapter.php';
...
class BarclaycardCw_Adapter_IframeAdapter extends BarclaycardCw_Adapter_AbstractAdapter {
	...
	protected function preparePaymentFormPane() {
		...
		if ($this->getTransaction() !== null) {
			if ($this->visibleFormFields !== null && count($this->visibleFormFields) > 0) {
				$link = new Link();
				$this->formActionUrl = $link->getModuleLink('barclaycardcw', 'iframe', array('cw_transaction_id' => $this->getTransaction()->getTransactionId(), 'id_module' => Tools::getValue('id_module', null)) , true);
			}
			else {
	...
	public function prepareWithFormData(array $formData, BarclaycardCw_Entity_Transaction $transaction) {
		...
		if ($transaction->getTransactionObject()->isAuthorizationFailed()) {
			...
			if (is_object($errorMessage)) {
				...
				$this->errorMessage = $this->l("Failed to initialize transaction with an unkown error");
			}
		}
	}
