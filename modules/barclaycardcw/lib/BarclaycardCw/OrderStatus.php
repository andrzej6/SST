...
class BarclaycardCw_OrderStatus {
	
	const LEGACY_SETTING_KEY = 'PENDING_ORDER_STATUS_ID';
	
	const SETTING_KEY = 'BARCLAYCARDCW_PENDING_STATE';
	
	private static $pendingOrderState = null;
	
	public static function getPendingOrderStatusId() {
		$result = BarclaycardCw::getInstance()->getConfiguraionValue(self::LEGACY_SETTING_KEY);
		if (!empty($result)) {
			return $result;
		}
		else {
	...
	public static function createPendingOrderState() {
		...
		$pendingOrderState->module_name = 'barclaycardcw';
		
		foreach (Language::getLanguages() as $language) {
			$pendingOrderState->name[$language['id_lang']] = 'Awaiting Payment (Barclaycard)';
		}
		...
		BarclaycardCw_OrderStatus::setPendingOrderStatusId($pendingOrderState->id);
