 ...
 * @Bean
 */
class BarclaycardCw_ConfigurationAdapter implements Customweb_Payment_IConfigurationAdapter{
	...
	public function getConfigurationValue($key, $languageCode = null) {
		...
		$rs = $this->getMainModule()->getConfiguraionValue($key, $langId);
	...
	public function existsConfiguration($key, $language = null) {
		...
		if ($language !== null) {
			...
			$langId = BarclaycardCw_Util::getLanguageIdByIETFTag($language);
		}

		return $this->getMainModule()->hasConfiguraionValue($key, $langId);
	}
	...
	public function getStoreHierarchy() {
		...
		if (!empty(Context::getContext()->cookie->shopContext)) {
			if (strpos(Context::getContext()->cookie->shopContext, 'g-') === 0) {
				...
				return array(
					'default' => BarclaycardCw::translate('Default'),
	...
	/**
	 * @return BarclaycardCw
	 */
	private function getMainModule() {
		return BarclaycardCw::getInstance();
	}
}