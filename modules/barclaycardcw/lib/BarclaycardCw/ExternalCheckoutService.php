 ...
 * @Bean
 */
class BarclaycardCw_ExternalCheckoutService extends Customweb_Payment_ExternalCheckout_AbstractCheckoutService {
	
	public function loadContext($contextId, $cache = true) {
		return $this->getEntityManager()->fetch('BarclaycardCw_Entity_ExternalCheckoutContext', $contextId, $cache);
	}
	
	public function renderShippingMethodSelectionPane(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages) {
		if (!($context instanceof BarclaycardCw_Entity_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('BarclaycardCw_Entity_ExternalCheckoutContext');
		}
		...
		return BarclaycardCw_Util::getTemplateRenderer()->render($templateContext);
	}
	...
	protected function updateShippingMethodOnContext(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request) {
		...
		// TODO: Can we apply this checks somehow differently? (without using the context?)
	...
	private function getShippingMethodNameFromContext(BarclaycardCw_Entity_ExternalCheckoutContext $context) {
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl) {
		...
		$externalCheckout = BarclaycardCw::getInstance()->getConfiguraionValue('external_checkout_account_creation');
	...
	protected function createTransactionContextFromContext(Customweb_Payment_ExternalCheckout_IContext $context) {
		...
		if (!($paymentMethod instanceof BarclaycardCw_PaymentMethod)) {
			throw new Customweb_Core_Exception_CastException('BarclaycardCw_PaymentMethod');
		}
		$orderContext = new BarclaycardCw_OrderContext(new Cart($context->getCartId()), new BarclaycardCw_PaymentMethodWrapper($paymentMethod));
	...
	public function getPossiblePaymentMethods(Customweb_Payment_ExternalCheckout_IContext $context) {
		...
		foreach (PaymentModule::getInstalledPaymentModules() as $module) {
			if (strpos($module['name'], 'barclaycardcw_') === 0) {
	...
	public function validateReviewForm(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request) {
		...
		if ($sessionContext->cookie->cart_total_amount != $cart->getOrderTotal()) {
			throw new Exception(BarclaycardCw::translate("Cart content was modified."));
		}
		
		$parameters = $request->getParameters();
		if ((int)(Configuration::get('PS_CONDITIONS')) && (!isset($parameters['cgv']) || $parameters['cgv'] !== '1')) {
			throw new Exception(BarclaycardCw::translate("You need to accept the general terms and conditions."));
		}
	}
	...
	private function updateMessage($messageContent, Cart $cart) {
		if (!empty($messageContent)) {
			if (!Validate::isMessage($messageContent)) {
				throw new Exception(BarclaycardCw::translate('Invalid message'));
			}
