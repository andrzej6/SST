...
class BarclaycardCw_ConfigurationApi {
	...
	public function getConfigurationValue($key, $languageId = null) {
		...
		if (strpos($rs, self::ASSET_IDENTIFIER) === 0) {
			...
			$rs = BarclaycardCw_Util::getAssetResolver()->resolveAssetStream($asset);
		}
	...
	public function getConfigurationKey($key, $languageId = null) {
		$key = 'BARCLAYCARDCW_' . $this->moduleId . '_' . $key;
		if ($languageId !== null) {
			$key .= '_' . $languageId;
		}
		...
		foreach ($fields as $fieldId => $field) {
			if ($field['type'] == 'orderstatus') {
				...
				if ($value instanceof Customweb_Core_Stream_Input_File) {
					$field['desc'] .= '<br />' . $this->l("Current: ") . $value->getFilePath();
				}
				$newFields[$fieldId] = $field;
				$newField = array(
					'type' => 'checkbox',
					...
					'values' => array(
						...
						'query' => array(
							'reset' => array(
								'val' => 'RESET',
								'name' => $this->l("Reset '@label' to default", array('@label' => $field['label'])),
								'id' => 'RESET',
							),
						),
					),
	...
	public function l($string, $specific = false) {
		return BarclaycardCw::translate($string, $specific);
	}
