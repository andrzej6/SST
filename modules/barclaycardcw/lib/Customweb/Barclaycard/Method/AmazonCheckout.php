...
require_once 'Customweb/Barclaycard/Method/LineItemBuilder/AmazonCheckout.php';
 ...
 */
class Customweb_Barclaycard_Method_AmazonCheckout extends Customweb_Barclaycard_Method_DefaultMethod {
	 ...
	 * (non-PHPdoc)
	 *
	 * @see Customweb_Barclaycard_Method_DefaultMethod::processAuthorization()
	 */
	public function processAuthorization(Customweb_Barclaycard_AbstractAdapter $adapter, Customweb_Barclaycard_Authorization_Transaction $transaction, $parameters){
		...
		//handle step two
		$builder = new Customweb_Barclaycard_Authorization_PaymentPage_ParameterBuilder($transaction, $this->getContainer(), array());
		$params = $builder->buildParameters();
		
		$url = $this->getGlobalConfiguration()->getBaseEndPointUrl() . Customweb_Barclaycard_AbstractAdapter::URL_PAYMENT_PAGE;
	...
	private function addSecondStepParameters(Customweb_Barclaycard_Authorization_Transaction $transaction){
		//Line items
		$builder = new Customweb_Barclaycard_Method_LineItemBuilder_AmazonCheckout($transaction->getTransactionContext()->getOrderContext());
	...
	private function isSecondStep(Customweb_Barclaycard_Authorization_Transaction $transaction){
