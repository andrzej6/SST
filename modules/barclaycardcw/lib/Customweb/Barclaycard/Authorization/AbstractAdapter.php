	...
	public function authorize(Customweb_Payment_Authorization_ITransaction $transaction, array $parameters) {		
		...
		// In all other cases we have to send the authorization request to the remote side
			...
			try {
				...
				if($this->getPaymentMethodByTransaction($transaction) instanceof  Customweb_Barclaycard_Method_DirectDebit_Server_Abstract && isset($parameters['CARDNO'])){
					$parameters['CARDNO'] = Customweb_Barclaycard_Util::maskIban($parameters['CARDNO']);
				}
