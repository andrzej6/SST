...
require_once 'Customweb/Barclaycard/Container.php';
...
require_once 'Customweb/Barclaycard/ExternalCheckout/MasterPass/AddressRequestParameterBuilder.php';
require_once 'Customweb/Barclaycard/ExternalCheckout/MasterPass/RedirectionRequestParameterBuilder.php';
require_once 'Customweb/Barclaycard/ExternalCheckout/MasterPass/AuthorizeRequestParameterBuilder.php';
 ...
 */
class Customweb_Barclaycard_ExternalCheckout_MasterPass_Endpoint extends Customweb_Payment_ExternalCheckout_AbstractCheckoutEndpoint {
	
	/**
	 *
	 * @var Customweb_Barclaycard_Container
	 */
	private $container;

	public function __construct(Customweb_DependencyInjection_IContainer $container){
		parent::__construct($container);
		$this->container = new Customweb_Barclaycard_Container($container);
	}
	...
	public function redirectAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$builder = new Customweb_Barclaycard_ExternalCheckout_MasterPass_RedirectionRequestParameterBuilder($context, $this->container, $this->getSecurityTokenFromRequest($request));
	...
	public function updateContextAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$builder = new Customweb_Barclaycard_ExternalCheckout_MasterPass_AddressRequestParameterBuilder($context, $this->container, $parameters);
			$responseParameters = $this->sendMessageToRemoteHost($builder->build());
	...
	public function confirmationAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$templateContext->addVariable('javascript', $this->getAjaxJavascript('.barclaycard-masterpass-shipping-pane', '.barclaycard-masterpass-confirmation-pane'));
	...
	private function authorizeTransaction(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Barclaycard_Authorization_Transaction $transaction){
		$this->getTransactionHandler()->beginTransaction();
		try {
			$builder = new Customweb_Barclaycard_ExternalCheckout_MasterPass_AuthorizeRequestParameterBuilder($context, $this->container);
	...
	private function getRemoteUrl(){
		return $this->container->getConfiguration()->getBaseEndPointUrl() . Customweb_Barclaycard_IAdapter::URL_DIRECT_ORDER;
	}
	...
	protected function getPaymentMethodByTransaction(Customweb_Barclaycard_Authorization_Transaction $transaction){
	...
	private function sendMessageToRemoteHost(array $parameters) {
		...
		$responseParameters = Customweb_Barclaycard_Util::getXmlAttributes($response->getBody());
