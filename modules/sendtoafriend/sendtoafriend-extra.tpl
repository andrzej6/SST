	...
	$('#sendEmail1').click(function(){
		...
		if (name && email && !isNaN(id_product))
		{
			$.ajax({
				...
				type: "POST",
				headers: {"cache-control": "no-cache"},
				data: {action: 'sendToUs', secure_key: '{/literal}{$stf_secure_key}{literal}', email: email, message: message,  id_product: id_product},{/literal}{literal}
				dataType: "json",
				success: function(result) {
					$.fancybox.close();
                    var msg = result ? "{/literal}{l s='Your e-mail has been sent successfully' mod='sendtoafriend'}{literal}" : "{/literal}{l s='Your e-mail could not be sent. Please check the e-mail address and try again.' mod='sendtoafriend'}{literal}";
                    var title = "{/literal}{l s='Send email about product' mod='sendtoafriend'}{literal}";
                    fancyMsgBox(msg, title);
				}
			});
		}
