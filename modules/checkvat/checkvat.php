...
?>';
			...
			foreach ($liste_customer_pro as $value_customer)
				...
				'style="background-color:#FAE2E3"' : 'style="background-color:#DFFAD3"' ).'>
					...
					<td>
						...
						'.Tools::strtoupper(Language::getIsoById($this->context->language->id)).'&memberStateCode=
						'.(($value_customer['iso_code'] == 'GR')? 'EL' : $value_customer['iso_code']).'&number=
						'.( Tools::substr($value_customer['vat_number'], 2)).
						(($value_customer['iso_code'] == 'ES' || $value_customer['iso_code'] == 'GR')? '&traderName=' : '').'">
	...
	*/
	...
	{
		$url = 'http://ec.europa.eu/taxation_customs/vies/vatResponse.html?locale=FR&memberStateCode='.(($iso_code == 'GR')?
				'EL' : $iso_code).'&number='.$vat.'&traderName=';
