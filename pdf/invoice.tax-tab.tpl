		...
		<td style="width: 83%">
			...
			<table style="width: 78%" >
				<tr style="line-height:5px;">
					<td style="text-align: left; background-color: #4D4D4D; color: #FFF; padding-left: 10px; font-weight: bold; width: 20%">{l s='VAT Detail' pdf='true'}</td>
					<td style="text-align: center; background-color: #4D4D4D; color: #FFF; padding-left: 10px; font-weight: bold; width: 20%">{l s='VAT Rate' pdf='true'}</td>
					{if !$use_one_after_another_method}
						<td style="text-align: center; background-color: #4D4D4D; color: #FFF; padding-left: 10px; font-weight: bold; width: 20%">{l s='Total' pdf='true'} <br/>
						{l s='(ex VAT)' pdf='true'}
						</td>
					{/if}
					<td style="text-align: right; background-color: #4D4D4D; color: #FFF; padding-left: 10px; font-weight: bold; width: 20%">{l s='Total VAT' pdf='true'}</td>
				</tr>
				...
				{if isset($product_tax_breakdown)}
					...
					<tr style="line-height:6px;background-color:{cycle values='#FFF,#DDD'};">
					 ...
					 <td style="width: 20%; text-align: center;">{$rate|string_format:"%d"} %</td>
					{if !$use_one_after_another_method}
					...
					<tr style="line-height:6px;background-color:{cycle values='#FFF,#DDD'};">
					 ...
					 <td style="width: 20%; text-align: center;">{$shipping_tax_infos.rate|string_format:"%d"} %</td>
					 
					{if !$use_one_after_another_method}
						 <td style="width: 20%; text-align: center;">{if isset($is_order_slip) && $is_order_slip}- {/if}{displayPrice currency=$order->id_currency price=$shipping_tax_infos.total_tax_excl}</td>
					{/if}
				...
				{if isset($ecotax_tax_breakdown)}
					{foreach $ecotax_tax_breakdown as $ecotax_tax_infos}
						...
						<tr style="line-height:6px;background-color:{cycle values='#FFF,#DDD'};">
							<td style="width: 20%">{l s='Ecotax' pdf='true'}</td>
							<td style="width: 20%; text-align: center;">{$ecotax_tax_infos.rate|string_format:"%d"  } %</td>
							{if !$use_one_after_another_method}
								<td style="width: 20%; text-align: center;">{if isset($is_order_slip) && $is_order_slip}- {/if}{displayPrice currency=$order->id_currency price=$ecotax_tax_infos.ecotax_tax_excl}</td>
							{/if}
