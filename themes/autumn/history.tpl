...
<h1 class="page-header">{l s='Order history'}</h1>
<h3 class="extra-info">{l s='Here are the orders you\'ve placed since your account was created.'}</h3>
		...
		<table id="order-list" class="table table-bordered footab">
			<thead>
				<tr>
					...
					<th class="item" data-hide="phone,tablet">{l s='Date'}</th>
					...
					<th data-hide="phone" class="item">{l s='Status'}</th>
					...
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						...
						<td class="history_invoice">
							{if (isset($order.invoice) && $order.invoice && isset($order.invoice_number) && $order.invoice_number) && isset($invoiceAllowed) && $invoiceAllowed == true}
								<a href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Invoice'}" target="_blank">
									<span class="wpicon wpicon-download"></span>&nbsp;{l s='PDF'}
								</a>
							{else}
								<a href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Invoice'}" target="_blank">
									<span class="wpicon wpicon-download"></span>&nbsp;{l s='PDF'}
						...
						<td class="history_detail">
							...
							<!--
							<a class="button-2 fill inline order-detail-button" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}');">
								<span class="wpicon wpicon-search"></span>{l s='Details'}
							</a>
							-->
							{if isset($opc) && $opc}
								<a class="button-1 fill inline order-reorder-button" href="{$link->getPageLink('order-opc', true, NULL, "submitReorder&id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
							{else}
								<a class="button-1 fill inline order-reorder-button" href="{$link->getPageLink('order', true, NULL, "submitReorder&id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
							{/if}
		                        <span class="wpicon wpicon-undo"></span>{l s='Reorder'}
							</a>
						</td>
					</tr>
				{/foreach}
