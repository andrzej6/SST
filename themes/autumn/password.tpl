...
{capture name=path}
    <a href="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" title="{l s='Authentication'}" rel="nofollow">{l s='Authentication'}</a>
...
<div class="row forgotpass-page-wrapper">
    ...
    {include file="$tpl_dir./errors.tpl"}
        ...
        {else}
            <form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="box inline" id="form_forgotpassword">
                <h3 class="section-header">{l s='Password recovery'}</h3>
                <div class="form-group">{l s='Please enter the email address you used to register. We will then send you a new password.'}</div>
                ...
                <div class="form-group submit">
                    <button type="submit" class="button-1 fill">{l s='Retrieve Password'}</button>
                </div>
            </form>
        {/if}
...
<ul class="footer_links">
	<li>
        <a href="{$link->getPageLink('authentication')|escape:'html':'UTF-8'}" title="{l s='Back to Login'}" rel="nofollow">
            <span class="wpicon wpicon-chevron-left small"></span>{l s='Back to Login'}
        </a>
    </li>
</ul>
