...
<h1 class="page-header">{l s='My loyalty points' mod='loyalty'}</h1>

{if $orders}
    <div class="block-center" id="block-history">
        {if $orders && count($orders)}
        <table id="order-list" class="table table-bordered wpresponsive">
            <thead>
                <tr>
            ...
            <tfoot>
                <tr class="alternate_item">
                    <td colspan="2" class="history_method bold">{l s='Total points available:' mod='loyalty'}</td>
                    <td class="history_method price">{$totalPoints|intval}</td>
        ...
        {if $nbpagination < $orders|@count}
            ...
            {if $page != 1}
                ...
                <li id="pagination_previous">
                    <a class="button-2 fill" href="{summarypaginationlink p=$p_previous n=$nbpagination}" title="{l s='Previous' mod='loyalty'}" rel="nofollow">{l s='Previous' mod='loyalty'}</a>
                </li>
            {else}
                <li id="pagination_previous"><span class="button-2 fill disabled">{l s='Previous' mod='loyalty'}</span></li>
            {/if}
            {if $page > 2}
                <li><a class="button-2 fill" href="{summarypaginationlink p='1' n=$nbpagination}" rel="nofollow">1</a></li>
                {if $page > 3}
            ...
            {section name=pagination start=$page-1 loop=$page+2 step=1}
                {if $page == $smarty.section.pagination.index}
                    <li class="current"><span class="button-2 fill disabled">{$page|escape:'html':'UTF-8'}</span></li>
                {elseif $smarty.section.pagination.index > 0 && $orders|@count+$nbpagination > ($smarty.section.pagination.index)*($nbpagination)}
                    <li><a class="button-2 fill" href="{summarypaginationlink p=$smarty.section.pagination.index n=$nbpagination}">{$smarty.section.pagination.index|escape:'html':'UTF-8'}</a></li>
                {/if}
            {/section}
            {if $max_page-$page > 1}
                {if $max_page-$page > 2}
                    <li class="truncate">...</li>
                {/if}
                <li><a class="button-2 fill" href="{summarypaginationlink p=$max_page n=$nbpagination}">{$max_page}</a></li>
            {/if}
            {if $orders|@count > $page * $nbpagination}
                {assign var='p_next' value=$page+1}
                <li id="pagination_next"><a class="button-2 fill" href="{summarypaginationlink p=$p_next n=$nbpagination}" title="Next" rel="nofollow">{l s='Next' mod='loyalty'}</a></li>
            {else}
                <li id="pagination_next" class="disabled"><span class="button-2 fill disabled">{l s='Next' mod='loyalty'}</span></li>
            {/if}
            </ul>
        {/if}
        ...
        {*{if $orders|@count > 10}
            <form action="{$pagination_link}" method="get" class="pagination">
                <p>
                    ...
                    <input type="submit" class="button-2 fill inline" value="{l s='OK'  mod='loyalty'}" />
                </p>
            </form>
        {/if}*}
    </div>
    ...
    {if $transformation_allowed}
        <p class="text-center">
            <a class="button-1 fill" href="{$link->getModuleLink('loyalty', 'default', ['process' => 'transformpoints'], true)|escape:'html':'UTF-8'}" onclick="return confirm('{l s='Are you sure you want to transform your points into vouchers?' mod='loyalty' js=1}');">{l s='Transform my points into a voucher of' mod='loyalty'} <span class="bold">{convertPrice price=$voucher}</span>.</a>
        </p>
    {/if}

    <h1 class="page-header">{l s='My vouchers from loyalty points' mod='loyalty'}</h1>
    ...
    {if $nbDiscounts}
        ...
        {if $minimalLoyalty > 0}
            <p>{l s='The minimum order amount in order to use these vouchers is:' mod='loyalty'} {convertPrice price=$minimalLoyalty}</p>
        {/if}
