...
{else}
	...
	<h1 class="page-header">{l s='Order summary' mod='paypal'}</h1>

	{assign var='current_step' value='payment'}
	{include file="$tpl_dir./order-steps.tpl"}

	<h3 class="section-header">{l s='PayPal payment' mod='paypal'}</h3>
	<form action="{$form_action|escape:'htmlall':'UTF-8'}" method="post" data-ajax="false">
		...
		<div class="cart_navigation">
			<input type="submit" name="confirmation" value="{l s='I confirm my order' mod='paypal'}" class="button-1 fill" />
		</div>
	</form>
{/if}
