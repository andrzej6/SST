...
<h1 class="page-header">{l s='Referral program' mod='referralprogram'}</h1>
...
<div>
    <h3 class="section-header t-align-left">{l s='Sponsor my friends' mod='referralprogram'}</h3>
    <h3 class="extra-info bold">{l s='Get a discount of %1$s for you and your friends by recommending this Website.' sprintf=[$discount] mod='referralprogram'}</h3>
    {if $canSendInvitations}
        ...
        <form method="post" action="{$link->getModuleLink('referralprogram', 'program', [], true)|escape:'html':'UTF-8'}" class="std">
            ...
            <p class="see_email">
                ...
                <a href="{$link->getModuleLink('referralprogram', 'email', ['height' => '500', 'width' => '600', 'mail' => {$file}], true)|escape:'html':'UTF-8'}" class="thickbox bold" title="{l s='Invitation e-mail' mod='referralprogram'}" rel="nofollow">{l s='the default e-mail' mod='referralprogram'}</a> {l s='that will be sent to your friend(s).' mod='referralprogram'}
            </p>
            <p class="submit">
                <button type="submit" id="submitSponsorFriends" name="submitSponsorFriends" class="button-1 fill">{l s='Validate' mod='referralprogram'}</button>
            </p>
        </form>
    {else}
...
<div>
    <h3 class="section-header secondary t-align-left">{l s='Pending friends' mod='referralprogram'}</h3>
    {if $pendingFriends AND $pendingFriends|@count > 0}
        <h3 class="extra-info">{l s='These friends have not yet placed an order on this Website since you sponsored them, but you can try again! To do so, mark the checkboxes of the friend(s) you want to remind, then click on the button "Remind my friend(s)"' mod='referralprogram'}</h3>
        <form method="post" action="{$link->getModuleLink('referralprogram', 'program', [], true)|escape:'html':'UTF-8'}" class="std">
            <table class="table table-bordered wpresponsive">
            <thead>
                <tr>
...
<div>
    <h3 class="section-header secondary t-align-left">{l s='Friends I sponsored' mod='referralprogram'}</h3>
    {if $subscribeFriends AND $subscribeFriends|@count > 0}
        <h3 class="extra-info">{l s='Here are sponsored friends who have accepted your invitation:' mod='referralprogram'}</h3>
        <table class="table table-bordered wpresponsive">
        <thead>
            <tr>
                <th class="first_item">&nbsp;</th>
