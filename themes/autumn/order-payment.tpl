...
{if !$opc}
	...
	<h1 class="page-header">{l s='Please choose your payment method'}</h1>
{else}
	<h1 class="order-opc-section-title">
        <span class="number">3</span>
        <span class="title">{l s='Please choose your payment method'}</span>
    </h1>
{/if}
			...
			<div id="HOOK_TOP_PAYMENT">{$HOOK_TOP_PAYMENT}</div>
							...
							<table id="cart_summary" class="table table-bordered wpresponsive">
								<thead>
									<tr>
										...
										<th class="cart_total last_item">{l s='Total'}</th>										{* <th class="cart_total last_item">{l s='Total (ex VAT)'}</th> *}
									</tr>
								</thead>
								<tfoot>
									{if $use_taxes}
										...
										{if $priceDisplay}
											...
											<tr class="cart_total_price">
												<td colspan="4" class="text-right">{if $display_tax_label}{l s='Total products'}{else}{l s='Total products'}{/if}</td>                                                 {* <td colspan="4" class="text-right">{if $display_tax_label}{l s='Total products (ex VAT)'}{else}{l s='Total products'}{/if}</td> *}
												<td colspan="2" class="special-alcels">
												   <div class="price" id="total_product">{displayPrice price=$total_products} (ex VAT) </div>
									...
									<tr class="cart_total_voucher" {if $total_wrapping == 0}style="display:none"{/if}>
										<td colspan="4" class="text-right">
											{if $use_taxes}
												{if $priceDisplay}
													{if $display_tax_label}{l s='Total gift wrapping (ex VAT):'}{else}{l s='Total gift wrapping cost:'}{/if}													
												{else}
										...
										{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
											{if $priceDisplay}
												<tr class="cart_total_delivery" {if $shippingCost <= 0} style="display:none"{/if}>
													<td colspan="4" class="text-right">{if $display_tax_label}{l s='Total shipping'}{else}{l s='Total shipping'}{/if}</td>                                                     {* <td colspan="4" class="text-right">{if $display_tax_label}{l s='Total shipping (ex VAT)'}{else}{l s='Total shipping'}{/if}</td> *}
													<td colspan="2" class="price" id="total_shipping">{displayPrice price=$shippingCostTaxExc} (ex VAT)																										<div id="tot-ship-incvat">{displayPrice price=$total_shipping} (inc VAT)</div>													</td>
												</tr>
											{else}
												<tr class="cart_total_delivery"{if $shippingCost <= 0} style="display:none"{/if}>
													<td colspan="4" class="text-right">{if $display_tax_label}{l s='Total shipping'}{else}{l s='Total shipping'}{/if}</td>                                                   {* <td colspan="4" class="text-right">{if $display_tax_label}{l s='Total shipping (ex VAT)'}{else}{l s='Total shipping'}{/if}</td> *}
									...
									{if $use_taxes}
										{if $priceDisplay && $total_tax != 0}
											<tr class="cart_total_tax">
												<td colspan="4" class="text-right">{l s='Total VAT:'}</td>
										...
										<tr class="cart_total_price">
											...
											<td colspan="2" class="special-alcels" id="total_price_container">
												 ...
												 <div id="total_price" class="price">{displayPrice price=$total_price_without_tax} (ex VAT)</div>
									...
									<tr class="cart_total_price">
										...
										{/if}
										<td colspan="{if !$voucherAllowed}3{else}2{/if}" class="total_price_container">
											<span>{l s='Total'}</span>
										</td>
				...
				{if !$opc}
                    <div class="cart_navigation">
                        <a href="{$link->getPageLink('order', true, NULL, "step=2")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-2 fill inline">
                            <span class="wpicon wpicon-arrow-left3 small"></span>{l s='Previous'}
                        </a>
                    </div>
                {else}
