...
{if isset($orderRet)}
    <div id="order-order-return-details">
        ...
        <div class="order-return-info">
            <h3 class="section-header t-align-left">{l s='RE#'}{$orderRet->id|string_format:"%06d"} {l s='on'} {dateFormat date=$order->date_add full=0}</h3>
        ...
        <h3 class="section-header t-align-left">{l s='List of items to be returned:'}</h3>
        <div id="order-detail-content" class="table_block">
            <table class="table table-bordered footab">
                <thead>
                    <tr>
                        <th data-sort-ignore="true" class="first_item">{l s='Reference'}</th>
                        <th data-sort-ignore="true" class="item">{l s='Product'}</th>
                        <th data-sort-ignore="true" data-hide="phone" class="last_item">{l s='Quantity'}</th>
                    </tr>
                </thead>
                <tbody>
        ...
        {if $orderRet->state == 2}
        <h3 class="section-header t-align-left">{l s='Reminder'}</h3>
        <div class="order-return-reminder">
            <ul>
                <li>{l s='All merchandise must be returned in its original packaging and in its original state.'}</li>
                <li>{l s='Please print out the'} <strong><a href="{$link->getPageLink('pdf-order-return', true, NULL, "id_order_return={$orderRet->id|intval}")|escape:'html':'UTF-8'}">{l s='PDF return slip'}</a></strong> {l s='and include it with your package.'}</li>
