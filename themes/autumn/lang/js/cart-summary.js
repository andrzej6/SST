...
$(document).ready(function(){
	...
	$('#wrapper').on('click', '.voucher_name', function(e){
...
function updateCartSummary(json)
{
	...
	for (i in product_list)
	{
		...
		if (typeof(product_list[i].price_without_quantity_discount) !== 'undefined')
			...
			priceReductionPercent = ps_round((ps_round(product_list[i].price_without_quantity_discount) - ps_round(product_list[i].price_wt))/ps_round(product_list[i].price_without_quantity_discount) * -100);            priceredamount = ps_round(product_list[i].quantity*ps_round(ps_round(1.2*product_list[i].price_without_quantity_discount) - product_list[i].price_wt));
		var current_price = '';
		if (priceDisplayMethod !== 0)
		 {
			 current_price = formatCurrency(product_list[i].price, currencyFormat, currencySign, currencyBlank);
			 current_price1 = formatCurrency(product_list[i].price_wt, currencyFormat, currencySign, currencyBlank);
		}
		else
			current_price = formatCurrency(product_list[i].price, currencyFormat, currencySign, currencyBlank);
			current_price1 = formatCurrency(product_list[i].price_wt, currencyFormat, currencySign, currencyBlank);
		...
		if (reduction && typeof(initial_price) !== 'undefined')
			if (initial_price !== '' && product_list[i].price_without_quantity_discount > product_list[i].price)
				initial_price_text = '<span class="price-percent-reduction small"> You save: £ '+priceredamount+ '</span>';                  
		...
		if (priceDisplayMethod !== 0)
		{
			$('#product_price_' + key_for_blockcart).html('<div class="price">' + current_price + ' (ex VAT)</div>' + '<div class="priceincvat">' + current_price1 + ' (inc VAT)</div>'+ initial_price_text);
			if (typeof(product_list[i].customizationQuantityTotal) !== 'undefined' && product_list[i].customizationQuantityTotal > 0)			
			  ...
			  {
			   	$('#total_product_price_' + key_for_blockcart).html(formatCurrency(product_list[i].total, currencyFormat, currencySign, currencyBlank) + ' (ex VAT)');
				  $('#total_product_priceincvat_' + key_for_blockcart).html(formatCurrency(product_list[i].total_wt, currencyFormat, currencySign, currencyBlank) + ' (inc VAT)');
				}
	...
	if (priceDisplayMethod !== 0)
	   {
		    $('#total_product').html(formatCurrency(json.total_products, currencyFormat, currencySign, currencyBlank) + ' (ex VAT)');
		    $('#total_productincvat').html(formatCurrency(json.total_products_wt, currencyFormat, currencySign, currencyBlank) + ' (inc VAT)');
		  }
	else
	   {
		$('#total_product').html(formatCurrency(json.total_products, currencyFormat, currencySign, currencyBlank) + ' (ex VAT)');
	...
	$('#total_price').html(formatCurrency(json.total_price_without_tax, currencyFormat, currencySign, currencyBlank) + ' (ex VAT)');
	
	$('#total_priceincvat').html(formatCurrency(json.total_price, currencyFormat, currencySign, currencyBlank) + ' (inc VAT)');
	
	$('#total_price_without_tax').html(formatCurrency(json.total_price_without_tax, currencyFormat, currencySign, currencyBlank) + ' (ex VAT)');
	...
	if (json.total_shipping > 0)
	{
		if (priceDisplayMethod !== 0)
		   {
			  $('#total_shipping').html(formatCurrency(json.total_shipping_tax_exc, currencyFormat, currencySign, currencyBlank) + ' (ex VAT)');
			  $('#tot-ship-incvat').html(formatCurrency(json.total_shipping, currencyFormat, currencySign, currencyBlank) + ' (inc VAT)');
			}
		else
		{
			$('#total_shipping').html(formatCurrency(json.total_shipping_tax_exc, currencyFormat, currencySign, currencyBlank) + ' (ex VAT)');
