...
<h1 class="page-header">{l s='Product Comparison'}</h1>
					...
					<td class="ajax_block_product comparison_infos product-block product-{$product->id}">
						<div class="remove">
							<a class="cmp_remove" href="{$link->getPageLink('products-comparison', true)|escape:'html':'UTF-8'}" title="{l s='Remove'}" data-id-product="{$product->id}">
								Remove <span class="wpicon wpicon-close small"></span>
							</a>
						</div>
						<div class="product-image-block">
							<a class="product_image img-wrapper" href="{$product->getLink()|escape:'html':'UTF-8'}" title="{$product->name|escape:'html':'UTF-8'}">
								<img class="white-border-3px" src="{$link->getImageLink($product->link_rewrite, $product->id_image, 'atmn_normal')|escape:'html':'UTF-8'}" alt="{$product->name|escape:'html':'UTF-8'}" />
							</a>
						</div>
                            ...
                            <div class="button-container">
                                {if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
                                    {if ($product->quantity > 0 OR $product->allow_oosp)}
                                        <a class="ajax_add_to_cart_button button-1 fill inline" data-id-product="{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" title="{l s='Add to cart'}">
                                    ...
                                    {/if}
                                {/if}
                                <a class="lnk_view button-2 fill inline" href="{$product->getLink()|escape:'html':'UTF-8'}" title="{l s='View'}">
                                    {l s='View'}
                                </a>
                            </div>
						</div>
...
<ul class="footer_links">
    <li class="back-to-home">
        <a class="button-2 fill inline" href="/compare">
            <span class="wpicon wpicon-shuffle2 medium"></span>{l s='Back to Compare'}
        </a>
    </li>
</ul>