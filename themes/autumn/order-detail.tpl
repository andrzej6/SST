...
*}
<div id="order-order-return-details">

    <div class="box">

    {if isset($order)}
        <form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit clearfix">
            ...
            <div>
                <a href="#" onclick="$(this).closest('form').submit(); return false;" class="button-2 fill inline"><span class="wpicon wpicon-undo"></span>{l s='Reorder'}</a>
            </div>
        </form>
        </div>
        ...
        <div class="info-order box">
            {if $carrier->id}
                <p><strong>{l s='Carrier:'}</strong> {if $carrier->name == "0"}{$shop_name|escape:'html':'UTF-8'}{else}{$carrier->name|escape:'html':'UTF-8'}{/if}</p>
            {/if}
            ...
            {if $invoice AND $invoiceAllowed}
                <p>
                    <a class="button-2 fill inline" target="_blank" href="{$link->getPageLink('pdf-invoice', true)}?id_order={$order->id|intval}{if $is_guest}&secure_key={$order->secure_key}{/if}"><span class="wpicon wpicon-download"></span>{l s='Download your invoice as a PDF file.'}</a>
                </p>
            {/if}
        ...
        {if count($order_history)}
            <h1 class="page-header">{l s='Follow your order\'s status step-by-step'}</h1>
            <div class="table_block">
                <table class="detail_step_by_step table table-bordered wpresponsive">
                    <thead>
                        <tr>
        ...
        {if isset($followup)}
            <div id="order-shipping-track">
                <p class="bold">{l s='Click the following link to track the delivery of your order'}</p>
                <a class="button-1 fill inline" href="{$followup|escape:'html':'UTF-8'}">{$followup|escape:'html':'UTF-8'}</a>
            </div>
        {/if}

        <div class="adresses_bloc">
            <div class="grid{if $order->isVirtual()} grid-1{else} grid-2{/if}">
                <div class="item"{if $order->isVirtual()} style="display:none;"{/if}>
                    <ul class="address box">
                        <li><h3 class="section-header">{l s='Delivery address'} ({$address_delivery->alias})</h3></li>
                        {foreach from=$dlv_adr_fields name=dlv_loop item=field_item}
                            {if $field_item eq "company" && isset($address_delivery->company)}<li><span class="address_company">{$address_delivery->company|escape:'html':'UTF-8'}</span></li>
                            {elseif $field_item eq "address2" && $address_delivery->address2}<li><span class="address_address2">{$address_delivery->address2|escape:'html':'UTF-8'}</span></li>
                            {elseif $field_item eq "phone_mobile" && $address_delivery->phone_mobile}<li><span class="address_phone_mobile">{$address_delivery->phone_mobile|escape:'html':'UTF-8'}</span></li>
                            {else}
                ...
                <div class="item">
                    <ul class="address box">
                        <li><h3 class="section-header">{l s='Invoice address'} ({$address_invoice->alias})</h3></li>
                        {foreach from=$inv_adr_fields name=inv_loop item=field_item}
                            {if $field_item eq "company" && isset($address_invoice->company)}<li><span class="address_company">{$address_invoice->company|escape:'html':'UTF-8'}</span></li>
                            {elseif $field_item eq "address2" && $address_invoice->address2}<li><span class="address_address2">{$address_invoice->address2|escape:'html':'UTF-8'}</span></li>
                            {elseif $field_item eq "phone_mobile" && $address_invoice->phone_mobile}<li><span class="address_phone_mobile">{$address_invoice->phone_mobile|escape:'html':'UTF-8'}</span></li>
                            {else}
                                {assign var=address_words value=" "|explode:$field_item}
        ...
        <div id="order-detail-content" class="table_block">
            <table class="table table-bordered wpresponsive">
                <thead>
                    <tr>
                        {if $return_allowed}<th data-sort-ignore="true" class="first_item"><input type="checkbox" /></th>{/if}
                        <th data-sort-ignore="true" data-hide="phone,tablet" class="{if $return_allowed}item{else}first_item{/if}">{l s='Reference'}</th>
                        <th data-sort-ignore="true" class="item">{l s='Product'}</th>
                        <th data-sort-ignore="true" data-hide="phone,tablet" class="item">{l s='Quantity'}</th>
                        {if $order->hasProductReturned()}
                            <th data-sort-ignore="true" data-hide="phone,tablet" class="item">{l s='Returned'}</th>
                        {/if}
                        <th data-sort-ignore="true" data-hide="phone,tablet" class="item">{l s='Unit price'}</th>
                        <th data-sort-ignore="true" data-hide="phone,tablet" class="last_item">{l s='Total price'}</th>
                    </tr>
                </thead>
                <tfoot>
                    ...
                    {if $order->total_discounts > 0}
                        <tr class="item">
                            ...
                            <td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
                                <span class="price">{displayWtPriceWithCurrency price=$order->total_discounts currency=$currency convert=1}</span>
                            </td>
                        </tr>
                    {/if}
                    ...
                    {if $order->total_wrapping > 0}
                        <tr class="item">
                            ...
                            <td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
                                <span class="price">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
                            </td>
                        </tr>
                    {/if}
                    ...
                    <tr class="item">
                        ...
                        <td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
                            <span class="price">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
                        </td>
                    </tr>
                ...
                <tbody>
                            ...
                            <!-- Customized products -->
                                <tr class="item">
                                    ...
                                    <td>
                                        ...
                                        <div class="return_quantity_buttons clearfix">
                                            <a href="#" class="return_quantity_down button-2 fill icon-only"><span class="wpicon wpicon-minus small"></span></a>
                                            <a href="#" class="return_quantity_up button-2 fill icon-only"><span class="wpicon wpicon-plus small"></span></a>
                                        </div>
                                ...
                                {foreach $product.customizedDatas  as $customizationPerAddress}
                                    ...
                                    <tr class="alternate_item">
                                        ...
                                        <td colspan="{if $order->hasProductReturned()}3{else}2{/if}"></td>
                                    </tr>
                                    {/foreach}
                                {/foreach}
                            {/if}
                    ...
                    {foreach from=$discounts item=discount}
                        <tr class="item">
                            ...
                            <td>&nbsp;</td>
                            <td><span class="price">{if $discount.value != 0.00}-{/if}{convertPriceWithCurrency price=$discount.value currency=$currency}</span></td>
                            {if $return_allowed}
                            <td>&nbsp;</td>
                            {/if}
                        </tr>
        ...
        {if $return_allowed}
            <div id="returnOrderMessage">
                <h3 class="section-header t-align-left">{l s='Merchandise return'}</h3>
                <h3 class="extra-info">{l s='If you wish to return one or more products, please mark the corresponding boxes and provide an explanation for the return. When complete, click the button below.'}</h3>
                <p class="form-group">
                    <textarea class="form-control" name="returnText"></textarea>
                </p>
                <p class="form-group">
                    <button type="submit" name="submitReturnMerchandise" class="button-1 fill">{l s='Make an RMA slip'}</button>
        ...
        {if !$is_guest}
            {if count($messages)}
                <div class="order-messages">
                    <h3 class="section-header t-align-left">{l s='Messages'}</h3>
                    <div class="table_block">
            ...
            <form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
                <h3 class="section-header t-align-left">{l s='Add a message'}</h3>
                <h3 class="extra-info">{l s='If you would like to add a comment about your order, please write it in the field below.'}</h3>
                <p class="form-group product-selector">
                ...
                <p class="form-group">
                    <textarea class="form-control" name="msgText"></textarea>
                </p>
                <div class="submit">
                    <input type="hidden" name="id_order" value="{$order->id|intval}" />
                    <input type="submit" class="unvisible" name="submitMessage" value="{l s='Send'}"/>
                    <button type="submit" name="submitMessage" class="button-1 fill">{l s='Send'}</button>
                </div>
            </form>
        {else}
